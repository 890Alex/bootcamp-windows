  ...
  if(flippedNotDone.length == 2){
    ...
    if(thisCard.classList[0] == thatCard.classList[0]){ // a match
      ...
      setTimeout(function(){
        ...
        thatCard.style.backgroundColor = ''
        clickDisabled = false;
      }, 1000)
    }
    if(document.querySelectorAll('.done').length == COLORS.length){
      ...
      let savedScore = parseInt(localStorage.getItem('bestScore')) || Infinity;
      let thisScore = parseInt(score.innerText)
      if(thisScore < savedScore){
        bestScore.innerText = thisScore;
        localStorage.setItem('bestScore', thisScore)
      }
    }
  }
}
...
bestScore.innerText = localStorage.getItem('bestScore') || Infinity;
